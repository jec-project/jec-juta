...
class TestDecorator implements Decorator {
  decorate(target:any, key:string, descriptor:PropertyDescriptor,
           params:TestParams):any { return target; }
}
...
describe("Test", ()=> {
  ...
  before(()=> {
    ...
    connector = new TestConnector(JutaConnectorRefs.TEST_CONNECTOR_REF, TEST_DECORATOR);
    context = factory.create();
    CTXM.addContext(JutaConnectorRefs.TEST_CONNECTOR_REF, context);
    DCM.addConnector(connector, context);
  });

  after(()=> {
    CTXM.removeContext(JutaConnectorRefs.TEST_CONNECTOR_REF);
    DCM.removeConnector(JutaConnectorRefs.TEST_CONNECTOR_REF, context);
    connector = null;
    context = null;
  });
  ...
  describe("@Test", ()=> {
    ...
    let annotationSpy:any = chai.spy.on(TestAnnotation, "Test");

    it("should invoke the JcadContextManager with the JutaConnectorRefs.TEST_CONNECTOR_REF reference", function() {
      expect(ctxmSpy).to.have.been.called.with(JutaConnectorRefs.TEST_CONNECTOR_REF);
    });
    ...
    it("should invoke the DecoratorConnectorManager with the JutaConnectorRefs.TEST_CONNECTOR_REF reference and the correct JCAD context", function() {
      expect(dcmSpy).to.have.been.called.with(JutaConnectorRefs.TEST_CONNECTOR_REF, context);
    });
    
    it("should invoke the annotation decorator with the right parameters", function() {
      expect(annotationSpy).to.have.been.called.with(params.TEST_PARAMS);
    });
    
    it("should invoke the registered decorator with the right method name and parameters", function() {
      expect(decoratorSpy).to.have.been.called.with(KEY, params.TEST_PARAMS);
    });
  });
});
