...
/*!
 * This module constains utilities used by the AfterTest test suite.
 */

// Utilities:
const LOADER:ClassLoader = new DefaultClassLoader();
const VALID_CLASS:string = process.cwd() + "/utils/test-utils/classes/annotations/AfterTestClass";
...
export const KEY:string = "afterMethod";
...
export const initContext:Function = function():JcadContext {
  ...
  const connector = new TestConnector(JutaConnectorRefs.AFTER_CONNECTOR_REF, TEST_DECORATOR);
  const context:JcadContext = factory.create();
  DecoratorConnectorManager.getInstance().addConnector(connector, context);
  JcadContextManager.getInstance().addContext(JutaConnectorRefs.AFTER_CONNECTOR_REF, context);
  return context;
}
export const resetContext:Function = function(context:JcadContext):void {
  JcadContextManager.getInstance().removeContext(JutaConnectorRefs.AFTER_CONNECTOR_REF);
  DecoratorConnectorManager.getInstance().removeConnector(JutaConnectorRefs.AFTER_CONNECTOR_REF, context);
  context = null;
}
