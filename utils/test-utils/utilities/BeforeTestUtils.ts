...
const VALID_CLASS:string = process.cwd() + "/utils/test-utils/classes/annotations/BeforeTestClass";
...
export const KEY:string = "beforeMethod";
...
export const initContext:Function = function():JcadContext {
  ...
  const connector = new TestConnector(JutaConnectorRefs.BEFORE_CONNECTOR_REF, TEST_DECORATOR);
  const context:JcadContext = factory.create();
  DecoratorConnectorManager.getInstance().addConnector(connector, context);
  JcadContextManager.getInstance().addContext(JutaConnectorRefs.BEFORE_CONNECTOR_REF, context);
  return context;
}
export const resetContext:Function = function(context:JcadContext):void {
  JcadContextManager.getInstance().removeContext(JutaConnectorRefs.BEFORE_CONNECTOR_REF);
  DecoratorConnectorManager.getInstance().removeConnector(JutaConnectorRefs.BEFORE_CONNECTOR_REF, context);
  context = null;
}
